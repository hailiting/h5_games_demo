...
//定义一个Hero类
	...
	function EnemyTank (x,y,direct,color){
		...
		this.rebirth=function(){
			//在0到400中产生一个随机数  0
			var  new_enemy=Math.round(Math.random()*370);
			this.x=new_enemy;
			this.y=0;
			this.isLive=true;
		}
	...
	function drawTank(tank){
		//说明所有的坦克都要isLive这个属性
		//console.info("tank live is===>"+tank.isLive);
		if(tank.isLive){
		...
		if(enemyBullet.isLive){
			if(hero.isLive){
				...
				if(enemyBullet.x>=hero.x && enemyBullet.x <=hero.x+20 && 
					...
					//hero.isLive=false;
					 //把对象的所有属性滞空
					var remanentLife=hero.lifeStop();
					//该子弹也死亡
							...
							switch(enemyTank.direct){
								...
								case 2://敌人坦克向下
									if(heroBullet.x>=enemyTank.x&&heroBullet.x<=enemyTank.x+20
										...
										//enemyTank.isLive=false;  shift()
										//把对象的所有属性滞空
										var remanentLife=enemyTank.lifeStop();
										hitedEnemyTanksNums.push(j);
										console.log("dead==>"+enemyTank.id);
										//该子弹也死亡
										...
										bombs.push(bomb);
										if(remanentLife){
											//setTimeout(enemyTank.rebirth,1000);
                                            setTimeout(function(){
                                            	var num=hitedEnemyTanksNums.shift();
                                            	console.log("rebirth==>"+enemyTanks[num].id);
                                            	enemyTanks[num].rebirth();
                                            },1000);
