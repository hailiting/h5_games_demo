...
}(window), function () { function a(a, b) { function c() { ++d >= e && b(f) } var d = 0, e = 0, f = a instanceof Array ? [] : {}; for (var g in a) e++, f[g] = new Image, f[g].onload = c, f[g].onerror = c, f[g].onabort = c, f[g].src = a[g] } var b = [], c = { page: { loading: $("#loading"), index: $("#index"), game: $("#game"), result: $("#result") }, btn: { start: $(".btn-start") }, game: { player: $("#player"), good: $("#good"), shopping_cart: $("#shopping-cart"), paw: $("#paw"), cd: $("#countDown"), main: $("#main-box"), cart: $("#cart"), time: $("#gameTime"), intro: $("#intro"), pawHeader: $("#paw em").eq(0) }, result: { score: $("#score") } }, d = { imgUrl: "http://wbpkh5001.17c.cn/html5/h5mini/1111/img/share-icon.jpg", timeLineLink: "http://url.cn/Z5vu5p?fr=share", title: "鍙�11鎴樼伀閲嶇噧锛岀柉鐙傗€滈挬鈥濈墿纭濈儫鍐嶈捣銆�" }, e = "ontouchstart" in window || window.DocumentTouch && document instanceof DocumentTouch ? "touchstart" : "click", f = window.innerWidth, g = window.innerHeight, h = [59, 99, 189, 239, 399, 599, 799, 5599, 1e4, 14288], i = [.1, .22, .34, .52, .67, .78, .87, .93, .97, 1], j = [2400, 2300, 2100, 2100, 2e3, 1900, 1700, 1600, 1300, 1200], k = { _isAnim: 0, _v: 400, initEvent: function () { var a = this; c.page.game.on(e, function (b) { if (!a._stop && !a._isAnim) { a._isAnim = 1; var d = "click" == e ? b.pageX : b.originalEvent.changedTouches[0].pageX, f = "click" == e ? b.pageY : b.originalEvent.changedTouches[0].pageY, g = d - a.tx, h = a.ty - f, i = Math.atan2(g, h), j = 180 * i / Math.PI; c.game.pawHeader.attr("class", 0 > j ? "l" : ""); var k = d, l = f, m = parseInt(Math.sqrt(g * g + h * h)), n = 10; this._isMoveDone2 = 0, c.game.paw.css("transform", "rotate(" + j + "deg)").animate({ height: m }, a._v, function () { var b = c.game.good.offset(), d = c.game.good.width(), f = c.game.good.height(); k + n > b.left && k - n < b.left + d && l + n > b.top && l - n < b.top + f && c.game.good.stop(!0, !1).animate({ left: a.cartOffset.left, top: a.cartOffset.top }, a._v, function () { $(this).hide().css("top", 0), a.addScore.call(a, $(this).data("m")), a._isMoveDone1 = 1, a.shoot.call(a) }), $(this).animate({ height: "click" == e ? 160 : 100 }, a._v, function () { a._isMoveDone2 = 1, a.shoot.call(a) }), a._isAnim = 0 }) } }) }, start: function () { this.reset(), this.shoot(); var a = this; this.gameTimer = setInterval(function () { return c.game.time.html(--a._time), a._time < 1 ? (clearInterval(a.gameTimer), void a.gameOver.call(a)) : void 0 }, 1e3) }, shoot: function () { if (this._isMoveDone1 && this._isMoveDone2 && !this._gameOver) { this._isMoveDone1 = 0; var a, b, d = Math.random(), e = 0, f = parseInt(50 * d); .5 > d ? (a = { left: -200, top: f }, b = { left: this.mW }) : (b = { left: -200 }, a = { left: this.mW, top: f }); for (var g = 0; 10 > g; g++) if (d < i[g]) { e = g + 1; break } var k = h[e - 1], l = this, m = j[e - 1]; this.shootTimer = setTimeout(function () { c.game.good.attr("class", "g" + e).css(a).show().html("<span>锟�" + k + "</span>").data("m", k).animate(b, m, function () { $(this).css({ left: -200 }), l._isMoveDone1 = 1, l.shoot.call(l) }) }, 300) } }, addScore: function (a) { this._score += a, c.game.cart.html("<em>" + this._score + "</em>") }, reset: function () { $("#spn").hide(), d.title = "鍙�11鎴樼伀閲嶇噧锛岀柉鐙傗€滈挬鈥濈墿纭濈儫鍐嶈捣銆�", this._time = 60, this._score = 0, this._gameOver = 0, this._isMoveDone1 = 1, this._isMoveDone2 = 1, this.cartOffset = c.game.cart.position(), this.mW = c.game.main.width(), this.mOffset = c.game.main.offset(), this.tx = f / 2 + (480 > f ? 40 : 80), this.ty = g - (480 > f ? 30 : 60), this.th = c.game.main.height() - parseInt(c.game.paw.css("bottom").replace("px", "")), c.game.good.css("left", -200), this._stop = 0 }, gameOver: function () { $("#span").show(), this._gameOver = 1, clearTimeout(this.shootTimer), d.title = "鍙�11鎴樼伀閲嶇噧锛岀柉鐙傗€滈挬鈥濈墿銆傛垜鎶㈠埌浠峰€�" + this._score + "鍏冪殑鍟嗗搧锛屼粎姝や竴澶╁摝~", c.result.score.html(this._score), c.page.game.hide(), c.page.result.show() } }, l = { init: function () { c.game.main.height(g - 150), this.initEvent(), k.initEvent(); var a = /micromessenger/.test(window.navigator.userAgent.toLowerCase()) ? "http://mp.weixin.qq.com/s?__biz=MjM5NzAzODA0Ng==&mid=200533929&idx=1&sn=95644f093189b86639325100da7f0c2a#rd" : "http://266.com"; $("#more_game").attr("href", a) }, countDown: function () { var a = 4; c.game.intro.show(); var b = setInterval(function () { 0 > a ? (c.game.cd.html(5), c.game.intro.hide(), clearInterval(b), k.start()) : c.game.cd.html(a), a-- }, 1e3) }, initEvent: function () { var a = this; c.btn.start.on(e, function () { var b = $(this).data("type"); c.game.player.attr("class", b), c.page.result.find(".logo").attr("class", "logo " + b), c.page.index.hide(), c.page.game.show(), a.countDown(b), c.game.cart.html("<em>0</em>") }), $(window).on("touchmove", function () { return !1 }) } }; a(["img/bg.png", "img/cart.png", "img/cat.png", "img/catB.png", "img/dog.png", "img/dogB.png", "img/footer.png", "img/g1.png", "img/g2.png", "img/g3.png", "img/g4.png", "img/g5.png", "img/g6.png", "img/g7.png", "img/g8.png", "img/g9.png", "img/g10.png", "img/header.png", "img/logo.png", "img/paw.png"], function () { document.addEventListener("WeixinJSBridgeReady", function () { WeixinJSBridge && (WeixinJSBridge.on("menu:share:appmessage", function () { WeixinJSBridge.invoke("sendAppMessage", { img_url: d.imgUrl, link: d.timeLineLink, desc: d.title, title: d.title }, function () { b.push(["_trackEvent", "share", "appmessage"]) }) }), WeixinJSBridge.on("menu:share:timeline", function () { WeixinJSBridge.invoke("shareTimeline", { img_url: d.imgUrl, img_width: "120", img_height: "120", link: d.timeLineLink, desc: d.title, title: d.title }, function () { b.push(["_trackEvent", "share", "timeline"]) }) })) }, !1), l.init.call(l), $("#by-loading").hide() }) }();