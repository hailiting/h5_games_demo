...
function createSprites() {
  try {
    ...
    g.titlescreen = new Image();
    g.titlescreen.src = g.imageData[0].src;
    g.gameback[0] = new Image();
    g.gameback[0].src = g.imageData[1].src;
    g.gameback[1] = new Image();
    g.gameback[1].src = g.imageData[2].src;
    g.gameback[2] = new Image();
    g.gameback[2].src = g.imageData[3].src;
    g.splash = new Image();
    g.splash.src = "library/splash.png";
    g.playpause = new Image();
    g.playpause.src = "library/playpause.gif";

    for (var a = 0; a < 1; a++) {
      g.bg[a] = new backgroundimage(
        "bg" + a,
        "library/background" + a + ".gif",
        320,
        480,
        0,
        0,
  ...
  try {
    ...
    for (var a = 0; a < NUM.ITEMS; a++)
      m.items[a] = new sprite(
        "item" + a,
        "item",
        m.spritesheets["itemsheet"],
        0,
        0,
        2,
        0,
    ...
    for (var a = 0; a < NUM.BONUSES; a++)
      m.bonuses[a] = new sprite(
        "bonuses" + a,
        "bonus",
        m.spritesheets["bonussheet"],
        0,
        0,
        2,
        0,
    ...
    for (var a = 0; a < NUM.MONSTERS; a++)
      m.monsters[a] = new sprite(
        "monster" + a,
        "monster",
        m.spritesheets["monstersheet"],
        0,
        0,
        2,
        0,
    ...
    for (var a = 0; a < NUM.TEXTSPRITES; a++)
      m.textsprites[a] = new textsprite("textsprite" + a, -32, -32, "", 0.1);
  } catch (e) {
...
function touch(event) {
  ...
  var tx =
    event.pageX -
    (g.canvas.offsetParent ? g.canvas.parentNode.offsetLeft : 0) -
    o.w / 2;
  var ty =
    event.pageY -
    (g.canvas.offsetParent ? g.canvas.parentNode.offsetTop : 0) -
    o.h / 2;
...
function setLevel() {
  g.canvas.style.backgroundImage =
    "url(" + g.gameback[rnd(g.gameback.length) - 1].src + ")";
...
function itemCollision(o, m) {
  if (!o.visible || m.dead || m.dying > 0 || o.dying > 0 || g.pausemode > 0)
    return;
  ...
  if (ox + ow >= mx && oy >= my && ox + ow <= mx + mw && oy <= my + mh)
    c2 = true;
  if (ox <= mx + mw && oy + oh >= my && ox >= mx && oy + oh <= my + mh)
    c3 = true;
  if (
    ox + ow >= mx &&
    ox + ow <= mx + mw &&
...
function itemPlayerCollision(o, m) {
  // redundant just now
  if (
    !o.visible ||
    !m.visible ||
  ...
  try {
    ...
    switch (g.mode) {
      ...
      case "game":
        ...
        for (var a = 0; a < NUM.MONSTERS; a++) {
          if (m.monsters[a].falling) {
            moveMonster(m.monsters[a]);
          }
          draw(m.monsters[a]);
        }
  ...
  if (g.pausemode < 1 && !m.player.dying) {
    ...
    for (var a = 0; a < NUM.MONSTERS; a++) {
      if (!m.monsters[a].visible) {
        var e = m.monsters[a];
        g.monsterstartx += 100;
        if (g.monsterstartx > 248) g.monsterstartx = 48;
        e.visible = true;
        e.direction = 0;
...
function spawnItemChance(o) {
  ...
  if (rnd(100) < 10 && g.pausemode < 1) {
    ...
    y = 0;
    var row =
      parseInt(m.spritesheets["entitysheet"].height) /
      parseInt(m.spritesheets["entitysheet"].frameheight);
...
function spawnFireball(o) {
  ...
  if (o.x > g.canvaswidth) {
    return;
  }
  if (g.fireballcooldown > 2) {
    return;
  }
  if (g.mode != "game") {
    return;
...
function hiScore() {
  try {
    ...
    for (var b = 0; b < sInScore.length; b++)
      s += "" + sInScore.substring(b, b + 1);
...
function updateScore() {
  try {
    m.player.score += 50;
    if (m.player.score > m.player.targetscore)
      m.player.score = m.player.targetscore;
...
function scanInput(e) {
  ...
  switch (keypress) {
    ...
    case 37: // Left
      break;
    case 67: // C
      g.console.style.display =
        g.console.style.display == "none" ? "block" : "none";
      break;
    case 80: // P
      g.pausemode++;
    ...
    case 83: // S
      g.audiomode++;
      if (g.audiomode > 1) g.audiomode = 0;
      handleAudio();
      break;
    case 17: // CTRL
    case 90: // Z
