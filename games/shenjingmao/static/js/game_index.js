...
function callLoading(method) {
  ...
  if (_getLoadingDiv) {
    ...
    _getLoadingDiv.innerHTML = [
      '<img src="../resource/img/logo.png" style="margin-top:' +
        ((window.innerHeight - 269) >> 1) +
        'px;" />',
    ].join("");
  ...
  crossAjax({
    ...
    success: function (res) {
      ...
      if (res.flag == "f0") {
        ...
        if (
          loginCounter == 1 ||
          confirm("您尚未登录，游戏积分无法上传到排行榜，是否登录？")
        ) {
          location.href =
            server.PageLoginURL +
            "?service=" +
  ...
  crossAjax({
    ...
    success: function (result) {
      ...
      if (data.flag == "f0" || data.flag == "f1") {
        ...
        if (data.flag != "f0") {
          tem =
            '<div class="your_rank">您的成绩:<a id="login_check" href="javascript:"  style="text-decoration: underline;color: inherit;">登录后才能查看呦</a></div>';
        } else {
          if (data.ulevel == -1) {
            tem =
              '<div class="your_rank">您的成绩:' +
              data.uscore +
              "分 排名:千里之外</div>";
          } else {
            tem =
        ...
        if (data.count > 0) {
          for (var i = 0; i < data.items.length; i++) {
            ...
            tem +=
              ...
              "</a></span></div>" +
              '<div class="list_item_content">分数：' +
              item.game_score +
              "</div>" +
              "</div>" +
              "</div>";
        ...
        tem += "</div>";
        tem +=
          '<a ontouchstart="" class="w_button w_button_small fl" id="gobackgame" href="u.ali213.net" style="width: 50px; margin-top: 10px;">继续游戏</a>';
        tem +=
          '<a ontouchstart="" class="w_button w_button_small fr" href="' +
          rootIndex +
          'list.php" style="width: 50px; margin-top: 10px;">更多游戏</a>';
        tem +=
          '<a ontouchstart="" class="w_button w_button_small fr" href="' +
          rootIndex +
          "toplist.php?game_code=" +
          game_code +
          '" style="width: 50px; margin-top: 10px;margin-right:17px;">更多排名</a>';
        win = new DWindow({
          width: 300,
          title: "排行榜",
          content: tem,
...
window.addEventListener("load", function (e) {
  ...
  function _resetFrame() {
    ...
    if (_getScreenFrame) {
      ...
      if (_getScreenFrame.children[0]) {
        ...
        callLoading("open");
        _getScreenFrame.innerHTML =
          '<iframe src="' + _getScreenFrame.lang + '" scrolling="no"></iframe>';
        //监听框架事件
        _getScreenFrame.children[0].addEventListener(
          "load",
          function (e) {
      ...
      _getScreenFrame.style.marginTop =
        ((window.innerHeight - parseInt(_getScreenFrame.style.height)) >> 1) +
        "px";
    }
  ...
  function _callLandscape(desc) {
    ...
    if (_getLandscapeDiv) {
      ...
      _getLandscapeDiv.innerHTML = [
        '<img src="../resource/img/orientation-landscape-' +
          desc +
          '.png" style="margin-top:' +
  ...
  function _createTopMenu() {
    ...
    if (!_getTopMenu) {
      ...
      _getTopMenu.innerHTML = [
        ...
        '<span style="position:relative;display:inline-block;width:60%;text-align:center;"><a href="httpssssss://www.ali213.net"><img id="directionMenuBtnDuoPao" src="../resource/img/iconmap_topheader.png" /></a></span>',
  ...
  if (!_playTimer) {
    (_rafRun = function () {
      //UI界面requestAnimationFrame主循环逻辑
      ...
      _newDate = null;
      if (_rafRun) _playTimer = requestAnimationFrame(_rafRun);
    })();
  }
});
var DWindow = (function ($) {
  function DWindow(config) {
    ...
    this.$panel = $(
      '<div class="w_panel"><div class="w_title">' +
        this.title +
        (this.showClose ? '<a class="menuBtn3-icon"></a>' : "") +
        '</div><div class="w_content">' +
        this.content +
        "</div></div>"
    ).css({
      width: this.width,
