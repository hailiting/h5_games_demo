...
var SG = {
  ...
  showLoadScrn: function () {
    ...
    var displayLoadScrn = function () {
      ...
      if (typeof body != "undefined") {
        ...
        if (SG.debug)
          console.log(
            "show load-screen: body-tag not ready. retrying in " +
              SG.loadScrnTimer +
              "ms"
          );
  ...
  hideLoadScrn: function () {
    ...
    if (loadscrn) loadscrn.parentNode.removeChild(loadscrn);
  },
  ...
  loadJsFiles: function (files, callback) {
    ...
    if (files.length > 0) {
      ...
      if (SG.debug)
        console.log(
          "loading " + script.src + ", " + files.length + " files left."
        );
  ...
  trigger: function (data, callback) {
    ...
    switch (data.type) {
      case "start":
        d = { type: window.softgames.eventStartingGame };
        break;
      case "levelUp":
        d = { type: window.softgames.eventLevelUp, level: data.level };
        break;
      case "gameOver":
        d = { type: window.softgames.eventGameOver, score: data.score };
        break;
      case "gameCompleted":
        d = { type: window.softgames.eventGameCompleted, score: data.score };
        break;
      case "gamePause":
        d = { type: window.softgames.eventGamePause, state: data.state };
        break;
      case "gameRestart":
        d = { type: window.softgames.eventGameRestart };
        break;
      case "selectLevel":
        d = { type: window.softgames.eventSelectLevel, level: data.level };
        break;
      case "selectMainMenu":
        d = { type: window.softgames.eventSelectMainMenu };
        break;
      case "setSound":
        d = { type: window.softgames.eventSound, state: data.state };
        break;
    }
  ...
  initLangs: function (supported_languages) {
    ...
    var hasLanguages =
      Object.prototype.toString.call(supported_languages).toLowerCase() ==
      "[object array]";
  ...
  setOrientationHandler: function (handler) {
    if (sg_exists()) window.softgames.changeScreenOrientation = handler;

    // handler();
  },

  setResizeHandler: function (handler) {
    if (sg_exists()) window.softgames.changeScreenSize = handler;
  },
  ...
  loadVoyager: function () {
    ...
    //sgc.src = 'httpssssss://scotty-staging.softgames.de/assets/api/voyager.js';
  ...
  detectPortalUrl: function () {
    ...
    if (typeof url !== "string") urlhref = "httpssssss://m.softgames.de";
    else urlhref = "httpssssss://" + url;

    SG.portalURL = url;
  },
  getLogoUrl: function (size) {
    return "httpssssss://d1bjj4kazoovdg.cloudfront.net/assets/sg_ig_logo.png";
  },
};
