...
function stack_init() {
  ...
  tmp.style.height = logh + "px";
  tmp.style.width =
    extraleft[sttype[sts]] + stwidth[sts] + extraright / 2 + "px";
...
function stack_dothemove() {
  ...
  document.getElementById("stp_" + sts).style.marginLeft =
...
function handleclick() {
  ...
  if (stmargin[sts] + stwidth[sts] > stmargin[sts - 1] + stwidth[sts - 1]) {
    stwidth[sts] -=
      stmargin[sts] + stwidth[sts] - stmargin[sts - 1] - stwidth[sts - 1];
    hcf = "right";
  }
  if (stwidth[sts] <= 0) {
    hcf = "all";
  ...
  if (Math.abs(stwidth[sts] - stwidth[sts - 1]) < 3 && stwidth[sts] > 0) {
    ...
    tmp2[0] = "";
    tmp2[1] =
      "$('score_title').style.color='#ffffff'; $('box_perfect').style.marginTop='" +
      (tmp3 - 5) +
      "px';";
    tmp2[2] = "$('box_perfect').style.marginTop='" + (tmp3 - 10) + "px';";
    tmp2[3] = "$('box_perfect').style.marginTop='" + (tmp3 - 15) + "px';";
    tmp2[4] =
      "$('st_scrollarea').removeChild($('box_perfect')); $('score_title').style.color=''";
    ttt = Math.random();
    stispace[ttt] = tmp2;
    setTI(ttt, 50);
  }
...
function cutaronk(crid, crtype, crstart, crstays, crfrom) {
  ...
  if (crfrom == "right") {
    document
      .getElementById(crid)
      .getElementsByTagName("div")[0]
      .getElementsByTagName("div")[0].style.width = crstays + "px";
    document
      .getElementById(crid)
      .getElementsByTagName("div")[0]
      .getElementsByTagName("img")[0].style.marginLeft =
      extraleft[crtype] + crstays - extraright / 2 + "px";
    showaronk(crid, crtype, crstart - crstays, croriw - crstays);
    document.getElementById(crid).childNodes[1].style.marginLeft =
      crstays + "px";
    ...
    document
      ...
      extraleft[crtype] + croriw - crstays + 1 + "px";
    document
      .getElementById(crid)
      .getElementsByTagName("div")[0]
      .getElementsByTagName("div")[0].style.backgroundPosition =
      crstart + crstays - croriw + "px";
    document
      .getElementById(crid)
      .getElementsByTagName("div")[0]
      .getElementsByTagName("img")[1].style.marginLeft =
      croriw - crstays + "px";
    showaronk(crid, crtype, crstart, croriw - crstays);
    document.getElementById(crid).childNodes[1].style.zIndex =
      sts % 2 == 1 ? 14 : 16;
  }
  ...
  if (crfrom != "") {
    ...
    if (crfrom == "left" || crfrom == "right") {
      tmp[0] =
        ...
        "').style.marginTop='" +
        (sah - (sts + 1) * (logh - 4)) +
        "px';document.getElementById('" +
        crid +
        "').childNodes[1].style.marginTop='2px';  document.getElementById('" +
        crid +
        "').childNodes[0].style.zIndex=" +
        (sts % 2 ? 13 : 16) +
        ";";
      tmp[1] =
        "$('" +
        crid +
        "').childNodes[1].style.opacity='.75'; $('" +
        crid +
        "').childNodes[1].style.marginTop='7px';";
      tmp[2] =
        "$('" +
        crid +
        "').childNodes[1].style.opacity='.5';$('" +
        crid +
        "').childNodes[1].style.marginTop='12px';";
      tmp[3] =
        "$('" +
        crid +
        "').childNodes[1].style.opacity='.25';$('" +
        crid +
        "').childNodes[1].style.marginTop='17px';";
      tmp[4] =
        "$('" +
        crid +
      ...
      tmp[1] =
        "$('" +
        crid +
        "').childNodes[0].style.opacity='.75'; $('" +
        crid +
        "').childNodes[0].style.marginTop='7px';";
      tmp[2] =
        "$('" +
        crid +
        "').childNodes[0].style.opacity='.5'; $('" +
        crid +
        "').childNodes[0].style.marginTop='12px';";
      tmp[3] =
        "$('" +
        crid +
        "').childNodes[0].style.opacity='.25'; $('" +
        crid +
        "').childNodes[0].style.marginTop='17px';";
      tmp[4] =
        "$('" +
        crid +
        "').removeChild(document.getElementById('" +
    ...
    document.getElementById(crid).style.marginTop =
      sah - (sts + 1) * (logh - 4) + "px";
    document.getElementById(crid).childNodes[0].style.zIndex =
      sts % 2 ? 13 : 16;
    stack_init();
  }
}
...
function gameover() {
  ...
  if (stscore > theirbest) {
    ...
    theirbest = stscore;
    ialert(
      "<h1>游戏结束: " + stscore + "分</h1>厉害，你又破纪录了",
      "restartgame();"
    );
  } else {
...
var DFW = {
  appId: "",
  TLImg: "httpssssss://games.vdcom.cn/games/duimutou/icon.png",
  url: "httpssssss://games.vdcom.cn/games/duimutou/",
...
var onBridgeReady = function () {
  WeixinJSBridge.on("menu:share:appmessage", function (argv) {
    if (tit > 500) {
      tite = DFW.title + " 我的分数是" + tit + "分，求超越！";
    } else {
      tite = DFW.title;
    }
...
function updateShare(stscore) {
	imgUrl = 'httpssssss://games.vdcom.cn/games/duimutou/i/icon.png';
	lineLink = 'httpssssss://games.vdcom.cn/games/duimutou';
